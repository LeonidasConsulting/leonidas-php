# Ransomware Readiness Quiz Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a free 10-question client-side ransomware readiness quiz at `/resources/ransomware-quiz` that scores SMB visitors across four categories and shows a score ring, category bars, and per-question gap report.

**Architecture:** Single PHP file (`resources/ransomware-quiz.php`) with all quiz logic in embedded JavaScript. No AJAX backend — scoring is 100% client-side. Follows the same patterns as `resources/security-scorecard.php` (PHP template vars, DOM methods for dynamic content, mobile-first CSS).

**Tech Stack:** PHP (template variables + JSON-LD schema), vanilla JS (quiz engine + DOM manipulation), CSS (card animations, mobile breakpoints). No external dependencies.
...
| Action | File | Purpose |
|--------|------|---------|
| Create | `leonidas-php/resources/ransomware-quiz.php` | Complete quiz page |
| Modify | `leonidas-php/sitemap.php` | Add `/resources/ransomware-quiz` entry |
| Modify | `leonidas-php/services/cybersecurity.php` | Add gold quiz teaser below scorecard teaser |
| Modify | `leonidas-php/services/managed-it.php` | Add gold quiz teaser below scorecard teaser |

### Codebase conventions (read before starting)

- `$b` = `SITE_BASE` — always `''` in production, `/leonidas-php` locally. Prefix all internal links and asset paths with `<?= $b ?>`.
- `SITE_URL` — absolute URL constant. Use for `og:image` and JSON-LD `url` fields.
- Set all `$page_*` variables **before** `require_once dirname(__DIR__) . '/includes/header.php'`.
- `$page_css` — raw CSS string injected inside `<style>` in `<head>`. Do not include a `<style>` tag.
- `$page_json_ld` — raw `<script>` block(s) injected in `<head>`.
- All dynamic content must use `createElement` / `textContent` / `appendChild`. Never use innerHTML with computed or user-derived data.
- To clear a container's children, use `container.textContent = ''` (safe, no innerHTML needed).
- Footer: `require_once dirname(__DIR__) . '/includes/footer.php'` at end of page.

---

### Task 1: PHP scaffold, JSON-LD, and CSS

**Files:**
- Create: `leonidas-php/resources/ransomware-quiz.php`

- [ ] **Step 1: Create the file — PHP variables and JSON-LD**
...
- [ ] **Step 2: Add CSS (append to same file, before header require)**
...
Open `http://localhost/leonidas-php/resources/ransomware-quiz`.
Expected: nav and footer visible, no PHP errors, blank content area.
View source: three `application/ld+json` blocks in `<head>`, CSS inside `<style>` tag.

- [ ] **Step 4: Commit**

```bash
cd leonidas-php
git add resources/ransomware-quiz.php
git commit -m "feat: ransomware quiz scaffold — PHP vars, JSON-LD, CSS"
```

---

### Task 2: Hero + section containers

**Files:**
- Modify: `leonidas-php/resources/ransomware-quiz.php`

- [ ] **Step 1: Add hero and empty containers after `?>`**
...
- [ ] **Step 2: Add footer require at very end of file**
...
Reload `http://localhost/leonidas-php/resources/ransomware-quiz`.
Expected: Hero visible — headline "How Ready Is Your Business for a Ransomware Attack?", gold "Start the Quiz" button, no quiz section visible yet.

- [ ] **Step 4: Commit**

```bash
git add resources/ransomware-quiz.php
git commit -m "feat: ransomware quiz hero + section containers"
```

---

### Task 3: All 10 question cards

**Files:**
- Modify: `leonidas-php/resources/ransomware-quiz.php`

Replace `<!-- QUESTION CARDS — added in Task 3 -->` with the 10 cards below. Each answer button uses `data-q` (0-indexed question number), `data-val` (10/5/0), and `data-text` attributes. The JS engine reads these via `dataset` — no per-button onclick needed.

- [ ] **Step 1: Add Q1–Q3 (Backup & Recovery)**
...
- [ ] **Step 2: Add Q4–Q6 (Access Control)**
...
- [ ] **Step 3: Add Q7–Q10 (Patching & Training)**
...
Click "Start the Quiz" at `http://localhost/leonidas-php/resources/ransomware-quiz`.
Expected: Hero hides, Q1 appears ("How often are backups tested?"), progress bar empty, "Question 1 of 10" label, Back button disabled. No other cards visible.

- [ ] **Step 5: Commit**

```bash
git add resources/ransomware-quiz.php
git commit -m "feat: ransomware quiz — all 10 question cards"
```

---

### Task 4: Results section HTML

**Files:**
- Modify: `leonidas-php/resources/ransomware-quiz.php`

Replace `<!-- RESULTS CONTENT — added in Task 4 -->` with the markup below. JS fills `#cat-bars` and `#breakdown-list` dynamically in Task 6.

- [ ] **Step 1: Add results HTML inside `#results-section`**
...
```bash
git add resources/ransomware-quiz.php
git commit -m "feat: ransomware quiz results section HTML"
```

---

### Task 5: Quiz JS engine

**Files:**
- Modify: `leonidas-php/resources/ransomware-quiz.php`

Add the `<script>` block **before** `<?php require_once dirname(__DIR__) . '/includes/footer.php'; ?>`.

- [ ] **Step 1: Add QUESTIONS/CATEGORIES data, state, and utilities**
...
- [ ] **Step 2: Add navigation functions and event listeners**
...
// Single delegated listener — reads data-q, data-val, data-text from button
...
Click "Start the Quiz". Verify:
1. Q1 shows with category "Backup & Recovery", Back disabled
2. Clicking any answer shows Q2, counter reads "Question 2 of 10", progress bar fills
3. Back on Q3 shows Q2; open DevTools console, type `state.answers[2]` — should be `null`
4. Completing Q10 calls `showResults()` — console no errors (results section empty for now)

- [ ] **Step 4: Commit**

```bash
git add resources/ransomware-quiz.php
git commit -m "feat: ransomware quiz JS engine — state, navigation, progress"
```

---

### Task 6: Results JavaScript

**Files:**
- Modify: `leonidas-php/resources/ransomware-quiz.php`

Add the four results functions inside the existing `<script>` block, after `backBtn.addEventListener('click', goBack);` and before `</script>`.

- [ ] **Step 1: Add showResults**
...
- [ ] **Step 2: Add buildCategoryBars**

```javascript
function buildCategoryBars() {
  var container = document.getElementById('cat-bars');
  container.textContent = ''; // clears all children safely
...
- [ ] **Step 4: Add retakeQuiz, close script tag**
...
- [ ] **Step 5: Verify full quiz flow**

Complete quiz at `http://localhost/leonidas-php/resources/ransomware-quiz` using a mix of A, B, C answers.

After Q10 verify:
1. Results section appears, page scrolled to top, ring animates to correct value
2. Score counter counts up (verify manually: count A answers × 10, B × 5)
3. Score label + color match tier: 80+ green "Strong Posture", 60–79 yellow-green "Moderate Risk", 40–59 amber "High Risk", 0–39 red "Critical"
4. Category bars animate to correct fractions. Example: Q1=A(10), Q2=B(5), Q3=C(0) → "Backup & Recovery: 15 / 30" with amber bar
5. All 10 rows in breakdown with correct icons
6. B/C answers show red-bordered gap recommendation; A answers do not
7. "Retake Quiz" returns to hero; `state.answers` all null in console

- [ ] **Step 6: Commit**

```bash
git add resources/ransomware-quiz.php
git commit -m "feat: ransomware quiz results JS — ring, category bars, breakdown"
```

---

### Task 7: FAQ section + sitemap + service teasers + deploy

**Files:**
- Modify: `leonidas-php/resources/ransomware-quiz.php`
- Modify: `leonidas-php/sitemap.php`
- Modify: `leonidas-php/services/cybersecurity.php`
- Modify: `leonidas-php/services/managed-it.php`

- [ ] **Step 1: Add FAQ section after the closing `</section>` of `#results-section`**
...
- [ ] **Step 2: Add quiz URL to sitemap.php**

Find the security-scorecard line in `leonidas-php/sitemap.php` and add after it:
...
- [ ] **Step 3: Add quiz teaser to cybersecurity.php**

Find the closing `</section>` of `<!-- SECURITY SCORECARD CTA -->` in `services/cybersecurity.php` and add directly after:

```html
  <!-- RANSOMWARE QUIZ CTA -->
  <section class="py-16 px-6" style="background:rgba(5,5,16,0.4);">
    <div class="max-w-7xl mx-auto">
      <div class="fade-in" style="background:linear-gradient(135deg,rgba(212,168,67,0.06) 0%,rgba(212,168,67,0.02) 100%);border:1px solid rgba(212,168,67,0.18);border-radius:1.25rem;padding:2.5rem;display:flex;align-items:center;justify-content:space-between;gap:2rem;flex-wrap:wrap;">
        <div style="flex:1;min-width:260px;">
          <div style="font-size:0.68rem;font-weight:700;letter-spacing:0.18em;color:#D4A843;text-transform:uppercase;margin-bottom:0.75rem;">Free Risk Assessment</div>
          <h2 style="font-size:clamp(1.4rem,2.5vw,2rem);font-weight:900;letter-spacing:-0.02em;color:#FFFFFF;margin:0 0 0.75rem;">How prepared is your business for a ransomware attack?</h2>
          <p style="font-size:0.9rem;color:#9CA3AF;margin:0;line-height:1.6;">10 questions. Instant score. See exactly where your backup, access control, patching, and training fall short — free, no signup.</p>
        </div>
        <div style="flex-shrink:0;">
          <a href="<?= $b ?>/resources/ransomware-quiz" style="display:inline-flex;align-items:center;gap:0.6rem;background:#D4A843;color:#0A0A1A;font-weight:700;font-size:0.9rem;padding:0.85rem 1.75rem;border-radius:0.6rem;text-decoration:none;transition:opacity 0.2s;" onmouseover="this.style.opacity='0.88'" onmouseout="this.style.opacity='1'">
            Take the Quiz
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7h10M8 3l4 4-4 4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </a>
        </div>
...
- [ ] **Step 4: Add same quiz teaser to managed-it.php**

In `services/managed-it.php`, find the closing `</section>` of `<!-- SECURITY SCORECARD CTA -->` and add the identical teaser HTML from Step 3 directly after it.

- [ ] **Step 5: Verify all wiring**

1. Bottom of `http://localhost/leonidas-php/resources/ransomware-quiz` — 5 FAQ cards and site footer visible
2. `http://localhost/leonidas-php/services/cybersecurity` — green "Scan My Domain" card, then gold "Take the Quiz" card
3. `http://localhost/leonidas-php/services/managed-it` — same two-card sequence
4. `http://localhost/leonidas-php/sitemap.xml` — `/resources/ransomware-quiz` entry present

- [ ] **Step 6: Final commit, push, and deploy**

```bash
cd leonidas-php
git add resources/ransomware-quiz.php sitemap.php services/cybersecurity.php services/managed-it.php
git commit -m "feat: ransomware quiz — FAQ, sitemap entry, service page teasers"
git push origin main
```

```bash
cd ..   # back to leonidas-site-starter/
python deploy-prep.py
mkdir -p leonidas-deploy/chat
cp leonidas-php/chat/blog-api.php leonidas-deploy/chat/blog-api.php
```

Upload to GoDaddy `public_html/` via WinSCP:
- `resources/ransomware-quiz.php`
- `services/cybersecurity.php`
- `services/managed-it.php`
- `sitemap.php`
- `chat/blog-api.php`

Post-upload smoke test on `leonidastek.com/resources/ransomware-quiz`:
1. Hero loads with "Start the Quiz" button
2. All 10 questions advance correctly, Back works
3. Results ring animates, category bars fill, all 10 breakdown rows appear
4. B/C answers show gap text; A answers do not
5. "Retake Quiz" resets to hero
6. "Book My Free Assessment" links to `/contact`
7. `leonidastek.com/sitemap.xml` includes `/resources/ransomware-quiz`
...
**Spec coverage:**
- ✅ One question at a time, progress bar, Back button (disabled on Q1)
- ✅ Results first, no lead gate
- ✅ 3-option multiple choice, A=10 B=5 C=0, all 10 questions
- ✅ Results: score ring + animated counter + 4 category bars + per-question breakdown + CTA
- ✅ Score tiers: 80+ green, 60-79 yellow-green, 40-59 amber, 0-39 red — colors consistent across ring, bars, icons
- ✅ Category maxes: Backup 30, Access 30, Patching 20, Training 20 = 100
- ✅ All 10 gap texts in QUESTIONS array
- ✅ FAQPage + BreadcrumbList + WebApplication JSON-LD
- ✅ 5 FAQ items, visible on page via PHP loop with htmlspecialchars, matching schema
- ✅ Mobile CSS: 48px min button height, font scaling, ring 170px at 640px
- ✅ DOM methods (createElement/textContent/appendChild) throughout results
- ✅ container.textContent = '' to clear containers (no innerHTML)
- ✅ Event delegation on quiz-section for answer buttons
- ✅ window.scrollTo({top:0}) on startQuiz and showResults
- ✅ Retake resets state.current=0 and state.answers to all-null array
- ✅ Back clears answers[current] before decrementing
- ✅ $b prefix on /contact link in CTA card
- ✅ Sitemap entry added
- ✅ Teasers on cybersecurity + managed-it (gold, distinct from green scorecard teasers)
- ✅ No server calls — pure client-side, no rate limiting needed
- ✅ og-cybersecurity.png reused for OG image
